...
public class Table implements java.io.Serializable, Cloneable {
     ...
     * @return
     */
    public String getClassNameBoLow(){
	return StringHelper.uncapitalize(getClassNameBo());
    }
    /**
     * 取得模块名 如 sys_user 则为sys
     * @author Guocg
     * @return
     */
    public String getModuleName() {
	
	String[] _sqlName = getSqlName().split("_");
	return _sqlName[0];
	...
	if (getPkColumns().isEmpty()) {
		//生成视图的时候没有主键，所以如果主键为空久直接返回nul 然后在share.include 进行判断null
//	    throw new IllegalStateException("not found primary key on table:"+getSqlName());
	...
	if (getColumnBySqlName(sqlName)==null) {
	    throw new IllegalArgumentException("not found column with sqlName:"+sqlName
		    +" on table:"+getSqlName());
	}
