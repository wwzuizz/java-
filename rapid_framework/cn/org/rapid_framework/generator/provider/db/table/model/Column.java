...
public class Column implements java.io.Serializable, Cloneable {
    ...
    public Column(Table table, int sqlType, String sqlTypeName, String sqlName, int size,
	    int decimalDigits, boolean isPk, boolean isNullable, boolean isIndexed,
	...
	_remarks = remarks;
	if (_remarks.startsWith("(")||_remarks.startsWith("（"))
	    _remarks = _remarks.substring(1, _remarks.length());
	if (_remarks.endsWith(")")||_remarks.endsWith("）"))
	...
	this(c.getTable(), c.getSqlType(), c.getSqlTypeName(), c.getSqlName(), c.getSize(), c
		.getDecimalDigits(), c.isPk(), c.isNullable(), c.isIndexed(), c.isUnique(), c
		.getDefaultValue(), c.getRemarks());
    }
	...
	return _remarks!=null ? _remarks.split("[,，(（#$]")[0] : null;
    }
    ...
    */
	...
	if (hasOne!=null&&_table!=null) {
	    ...
	    _table.getImportedKeys().addForeignKey(hasOne.tableName, hasOne.columnSqlName,
	...
	if (hasMany!=null&&_table!=null) {
	    ...
	    _table.getExportedKeys().addForeignKey(hasMany.tableName, hasMany.columnSqlName,
	...
	String normalJdbcJavaType = DatabaseDataTypesUtils.getPreferredJavaType(getSqlType(),
		getSize(), getDecimalDigits());
	javaType = GeneratorProperties.getProperty("java_typemapping."+normalJdbcJavaType,
	...
	columnAlias = StringHelper.removeCrlf(StringHelper.defaultIfEmpty(getRemarksFirst(),
